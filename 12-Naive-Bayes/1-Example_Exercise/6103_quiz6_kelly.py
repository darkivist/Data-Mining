...
#eliminate leading/trailing white space
...
#correct '?' values, 'South' country value, drop NA. 'South' is not a country; dropping this value as it is only about
#80 records.
...
#Note that removing observations with missing data can result in a biased model; however, initial checks of the data
#have 24% of original data is observations with >50K; 25% of 'cleaned' data is observations with >50K, suggesting the
#dataset has not been biased against target variable.
...
#This is a classification problem because classification models aim to categorize observations into one or more outcome
#categories based on variable values. In the adult dataset, we aim to predict whether a new observation will be an
#individual with an income of >50K based on the levels of their feature variables.
...
#After removal of unknowns and correcting variable values in the training and test sets, the resulting Naive Bayes model
#has an Accuracy of 78.42%. The ROC_AUC is 82.7. These are relatively good scores that could be improved with further
#tuning of features selected in the model. The confusion matrix shows that, of 9028 total observations in the testing
#set, 658 observations were correctly categorized as having income >50K, 6422 were correctly categorized as having an
#income <=50K; the combined total of these gives us the Accuracy score of 78%.
...
#Naive Bayes and Decision tree are both supervised learning models. Naive Bayes is a linear classification model. It can
#be tuned by two hyperparameters, alpha and beta. It is based on using the stastistical theorem, Bayes Theorem, assuming
#all variables are independent. Decision Tree is a flowchart-like structural classification model represented by
#decision nodes with rule sets dictating where an observation is categorized.